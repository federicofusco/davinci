...
const LoginForm = ({ action, onSubmit }: LoginFormProps ) => {
        ...
        const confirmPasswordRef: RefObject<HTMLInputElement> = createRef ();
        const callback = (event: FormEvent<HTMLFormElement>) => {
                event.preventDefault ();
                if ( emailRef.current && passwordRef.current && confirmPasswordRef.current ) onSubmit ( emailRef.current.value, passwordRef.current.value, confirmPasswordRef.current.value );
        };
        ...
        return (
                <form onSubmit={ callback }>
                        ...
                        <fieldset className="mt-3 w-full bg-transparent border border-slate-700 rounded-md px-2">
                                <legend className="px-2 text-slate-400 font-source">Confirm Password</legend>
                                <input ref={ confirmPasswordRef } className="w-full pb-2 font-source bg-transparent px-2 placeholder:font-source text-grayslate-400 text-sm text-slate-400 placeholder:text-sm mx-auto outline-none" placeholder="Super secret password again" type="password" />
                        </fieldset>
